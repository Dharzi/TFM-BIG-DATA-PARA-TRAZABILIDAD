...
def decodifica_transaction(transaccion):
    ...
    #Funcion deprecada por Transaction.parse
    #tx = bilib.transactions.transaction_deserialize(transaccion['hex'],network='bitcoin', check_size=True)

    # Parsea la transacción utilizando Transaction.parse()
    # El parametro strict=False evita que salga error si una transaccion está mal formada, incompleta o no se entiende
    # De esa forma solo se tratan las transacciones correctas
    tx = bilib.transactions.Transaction.parse(transaccion['hex'],strict=False, network='bitcoin')
    decoded_transaction = {
        'version': tx.version,
...
def cantidad_redondada(numero):
    """
    A partir de un numero flotante indica si esta resdoneado a apartir del quinto decimal

    Argumento:
        numero - numero flotante 
   
    Devuelve:
       true - La cantidad esta redondeada
       false - cantidad no redondeada
    """
 
    numero_redondeado = round(float(numero), 5)  # Redondea el número a 5 decimales
    
    if numero == numero_redondeado:
        return True  # El número está redondeado
    else:
        return False  # El número no está redondeado                
...
#Trata cada una de las transacciones del bloque actual y crea tantos wallets como sean necesarios
#con los input de cada transaccion, para esste estuio solo usa ls transacciones que tengan un 
#input y como maximo 2 ouput

print('')
print('TRATANDO TRANSACCIONES Y CREANDO WALLETS')
print('----------------------------------------')
while num_linea_array < tam_array_dictactual:
    print('   TRANSACCION %s de %s' %(num_linea_array, tam_array_dictactual))
...
print('')
print('TRATANDO WALLETS')
print('----------------')
while cont_wallet < tam_array_wallets:
    print("   WALLET: %s de %s" %(cont_wallet,tam_array_wallets))
...
#End while Wallet
print()
print("WALLETS GENERADOS",wallets_array)
print("-----------------")
